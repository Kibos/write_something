...
import static android.develop.utils.fastjson.util.ASMUtils.getDesc;
import static android.develop.utils.fastjson.util.ASMUtils.getType;
...
import android.develop.utils.fastjson.asm.ClassWriter;
import android.develop.utils.fastjson.asm.FieldVisitor;
import android.develop.utils.fastjson.asm.Label;
import android.develop.utils.fastjson.asm.MethodVisitor;
import android.develop.utils.fastjson.asm.Opcodes;
import android.develop.utils.fastjson.parser.Feature;
import android.develop.utils.fastjson.parser.ParserConfig;
import android.develop.utils.fastjson.util.ASMClassLoader;
...
public class ASMDeserializerFactory implements Opcodes {
    ...
    void _deserialzeArrayMapping(ClassWriter cw, Context context) {
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == byte.class //
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitLdcInsn(android.develop.utils.fastjson.asm.Type.getType(getDesc(fieldClass)));
                ...
                if (itemClass == String.class) {
                    ...
                    mw.visitVarInsn(ALOAD, 1);
                    mw.visitLdcInsn(android.develop.utils.fastjson.asm.Type.getType(getDesc(itemClass)));
    ...
    void _deserialze(ClassWriter cw, Context context) {
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == boolean.class) {
                ...
                if (itemClass == String.class) {
                    mw.visitLdcInsn(android.develop.utils.fastjson.asm.Type.getType(getDesc(fieldClass))); // cast
    ...
    private void _deserialze_list_obj(Context context, MethodVisitor mw, Label reset_, FieldInfo fieldInfo,
        ...
        mw.visitFieldInsn(GETFIELD, context.getClassName(), fieldInfo.getName() + "_asm_list_item_deser__",
                          "Lcom/alibaba/fastjson/parser/deserializer/ObjectDeserializer;");
        mw.visitVarInsn(ALOAD, 1);
        mw.visitLdcInsn(android.develop.utils.fastjson.asm.Type.getType(getDesc(itemType)));
    ...
    private void _deserObject(Context context, MethodVisitor mw, FieldInfo fieldInfo, Class<?> fieldClass) {
        ...
        if (fieldInfo.getFieldType() instanceof Class) {
            mw.visitLdcInsn(android.develop.utils.fastjson.asm.Type.getType(getDesc(fieldInfo.getFieldClass())));
        } else {
            mw.visitVarInsn(ALOAD, 0);
            mw.visitLdcInsn(fieldInfo.getName());
